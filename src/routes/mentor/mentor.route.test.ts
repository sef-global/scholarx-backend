...
describe('Register and login', () => {
	...
	describe('Create mentor route', () => {
		it('should return a 200 with a mentor object and the message', async () => {
			const response = await agent.post('/api/mentors').send(mentorApplicationInfo).expect(200)
		...
		it('should return a 409 when the user is already a mentor or has an pending invitation', async () => {
			const response = await supertest(server).post('/api/mentors').send(mentorApplicationInfo).expect(401)
		...
		it('should return a 404 when the category id is not valid', async () => {
			await supertest(server).post('/api/mentors').send({ ...mentorApplicationInfo, categoryId: uuidv4() }).expect(401)
		})
